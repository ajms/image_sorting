...
def cache_to_json(cache_dir: Path):
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            kwargs_extended = {
                param_name: v for param_name, v in zip(parameters.keys(), args)
            } | kwargs
            kwargs_str = json.dumps(
                {k: str(v) for k, v in sorted(kwargs_extended.items())}
            )
            print(f"{func_name=}, {kwargs_str=}")

            cache_filename = (
                cache_dir
                / f"{func_name}_{hashlib.sha256(kwargs_str.encode('utf-8')).hexdigest()}.json"
            )
...
def create_hash_dict(
    folder: Path, hash_function: callable = imagehash.dhash
) -> dict[str, Path]:
    hash_dict = {}
...
@cache_to_json(cache_dir=get_project_root() / "data/processed")
def create_delete_dict(hash_dict: dict[str, list[str]]) -> dict[str, list[str]]:
    duplicates = [k for k, v in hash_dict.items() if len(v) > 1]
    delete_dict = {}
    for k in duplicates:
        delete_dict[k] = extract_duplicates_for_deletion(hash_dict.pop(k))
    assert len(duplicates) == len(delete_dict)
    return delete_dict


def extract_duplicates_for_deletion(paths: list[str]):
    max_width = 0
    max_height = 0
    tmp_picture = None
    for path in paths:
...
def remove_duplicates(delete_dict=dict[str, list[str]]):
...
if __name__ == "__main__":
    ...
    delete_dict = create_delete_dict(hash_dict)

    remove_duplicates(delete_dict=delete_dict)
